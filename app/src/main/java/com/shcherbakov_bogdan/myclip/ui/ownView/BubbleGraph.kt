...
class BubbleGraph : View {
    ...
    constructor(context: Context, attrs : AttributeSet) : super(context, attrs) {
        ...
        circleFill?.color = typedArray.getColor(R.styleable.BubbleGraph_circle_fill_color,Color.BLUE)
        circleStroke?.color = typedArray.getColor(R.styleable.BubbleGraph_circle_border_color,Color.BLUE)
        typedArray.recycle()
    }
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        for (args in floatList) {
            canvas?.drawCircle(args.first.toFloat() + cx, args.second.toFloat() + cy, args.third.toFloat(), circleStroke!!)
        }
        for (args in floatList) {
            canvas?.drawCircle(args.first.toFloat() + cx, args.second.toFloat() + cy, args.third.toFloat() - strokeWidth, circleFill!!)
        }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        minVSize = mWidth.coerceAtMost(mHeight) //Вычисляем минимальный размер (будем рисовать круги по минимальному размеру View в высоте или ширине)
